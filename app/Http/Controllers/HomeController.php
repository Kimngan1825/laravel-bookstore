...
class HomeController extends Controller
{
    ...
    public function index() {
        ...
        $categories = Category::with(['books' => function($q) {
            $q->select('book_id', 'category_id', 'image')->limit(1);
        }])->get();

        $saleBooks = Book::where('discount', '>', 0)->orderBy('discount', 'desc')->take(5)->get();

        $bestSellers = Book::withCount(['orderItems as total_sold'])
            ->orderBy('total_sold', 'desc')
            ->take(8)->get(); 
        ...
        return view('components.index', compact('heroSlides', 'categories', 'saleBooks', 'top5Books', 'newBooks', 'bestSellers', 'top5Books'));
    }
